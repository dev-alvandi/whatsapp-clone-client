  ...
  public initAuthentication(): void {
    from(
      ...
      if (isAuthenticated) {
        ...
        this.authModalRef = this.modalService.open(AuthModalComponent, {
          centered: true,
          backdrop: 'static',
        });
      }
  ...
  handleLastSeen(userPublicId: string): void {
    ...
    this.http
      ...
      .subscribe({
        next: (lastSeen) =>
          this.lastSeen$.next(
            State.Builder<Dayjs>().forSuccess(dayjs(lastSeen))
          ),
        error: (err) =>
          this.lastSeen$.next(State.Builder<Dayjs>().forError(err)),
      });
  }
}
